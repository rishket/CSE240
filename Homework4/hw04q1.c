...
{/*
	...
	int i, l;
	char *begin_ptr, *end_ptr, temp; //Initializing begin and end pointers, as well as temp variable.

	l = strlen(s);
	begin_ptr = &s[0];
	end_ptr = &s[l-1]; //setting maddress locations for both pointers to beginning and end of string.
	for(i = 0; i < l/2; i++){
		temp = *end_ptr;
		*end_ptr = *begin_ptr;
		*begin_ptr = temp; //Performing the swap
r
		begin_ptr++; //increment
		end_ptr--;  //decrement
	}
...
void reverseStrings(char strings[NUM_STRINGS][STRING_LENGTH])
{
	...
	char* ptr = &strings[0][0];
	for(i = 0; i < NUM_STRINGS; i++){
		reverseOneString(strings[i]); //reversing each string in array
	}
}
...
{
	int i = 0;
	char* ptr = &strings[0]; //initialize pointer to start of array
	reverseStrings(ptr); // reversing string
	for(i = 0; i < NUM_STRINGS; i++){
		ptr = &strings[i];
		for(; *ptr != '\0'; ++ptr){
			*ptr = *ptr + key; // incrementing each pointer value by value of key
		}
	}
...
void decryptStrings(char strings[NUM_STRINGS][STRING_LENGTH], int key)
{
	...
	for(i = 0; i < NUM_STRINGS; i++){
		ptr = &strings[i];
		for(; *ptr != '\0'; ++ptr){
			*ptr = *ptr - key; //decrementing each pointer value by value of key
		}
	}
...
{
	char* ptr = s;
	char* last = ptr + strlen(s) - 1;
	int palindrome = 1; //initializing palindrome value to 1

	while(ptr < last){
		if (*ptr != *last){
			palindrome = 0; //sets to false if string is not palindrome
			break;
		}
		ptr++;
		last--;
